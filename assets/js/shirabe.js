(() => {
  ...
  document.querySelector("#next").addEventListener("click", () => { if (!validateStep()) return; if (step === 3 && !consistencyCheck()) { document.querySelector("#consistency-note").scrollIntoView({ behavior:"smooth", block:"center" }); return; } step += 1; renderStep(); window.scrollTo({ top: 0, behavior: "smooth" }); });
