...
const loginCredential = crypto.randomUUID();
let cookie;
...
beforeAll(async () => {
  ...
  const material = await crypto.subtle.importKey("raw", new TextEncoder().encode(loginCredential), "PBKDF2", false, ["deriveBits"]);
...
describe.sequential("Cypher Online document boundary", () => {
  ...
  it("authenticates the tenant secretary", async () => {
    const response = await SELF.fetch("https://cypher.test/api/cypher/v1/login", {
      ...
      body: JSON.stringify({ company: "aap-test", email: "secretary@example.test", password: loginCredential }),
    });
