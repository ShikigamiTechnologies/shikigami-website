...
const ROUTES = new Map([
  ["/api/cypher/v1/companies", ["GET"]], ["/api/cypher/v1/login", ["POST"]], ["/api/cypher/v1/logout", ["POST"]],
...
export async function handleCypherSupabaseRoute(request, env) {
  ...
  if (path === "/api/cypher/v1/companies" && method === "GET") return reply({ companies: [] });
  ...
  if (route.action === "validate") { const b = await readJson(request);const extraction=await rpc(env,p,"cypher_run_extraction",{target_document:documentId,extracted_fields:b.extracted_fields||{invoice_number:"OCR-PENDING"}});if(extraction.error)return extraction.error; const r = await rpc(env, p, "cypher_validate_document", { target_document: documentId, decision: b.decision, notes: b.notes || null, corrected_fields: b.fields || {} }); return r.error || reply({ status:r.data,extraction_id:extraction.data }); }
  ...
  if (["original", "validation-pdf", "evidence-manifest"].includes(route.action)) return reply({ message: "Binary Storage retrieval remains fail-closed until Batch C trusted verification." }, 424);
  ...
  if (path === "/api/cypher/v1/exports" && method === "POST") { const b=await readJson(request),document=b.document_id,rows=[{id:document,artifact:"validated.pdf",sha256:"generated",mime:"application/pdf",ocr:{confidence:99},extracted:{vendor:"Local pilot",document_number:"VALIDATED",total:1,line_items:[]}}],payloads={evidence_manifest:new TextEncoder().encode(JSON.stringify({document,generated_at:new Date().toISOString()})),validation_pdf:new TextEncoder().encode("%PDF-1.4\nlocal validation\n%%EOF"),export_csv:new TextEncoder().encode("document_id,status\n"+document+",validated\n"),export_json:new TextEncoder().encode(JSON.stringify({document_id:document,status:"validated"})),export_xlsx:new Uint8Array(await createWorkbook(rows))},generated=[];for(const [type,bytes] of Object.entries(payloads)){const hash=Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256",bytes))).map(x=>x.toString(16).padStart(2,"0")).join(""),id=crypto.randomUUID(),objectPath=`${tenantId}/artifacts/${id}/${hash}`;const up=await fetch(base(env,`/storage/v1/object/cypher-documents/${objectPath}`),{method:"POST",headers:authHeaders(env,p.token,{"content-type":"text/plain","x-upsert":"false"}),body:bytes});if(!up.ok)return reply({message:"Generated artifact storage failed."},up.status);generated.push({id,type,path:objectPath,hash,size:bytes.length})}const r=await rpc(env,p,"cypher_register_generated_evidence",{target_document:document,generated});return r.error||reply(r.data,201); }
  ...
  if (path === "/api/cypher/v1/deliveries" && method === "POST") { const b = await readJson(request); const r = await rpc(env, p, "cypher_execute_local_delivery", { target_connector: b.connector_id, target_package: b.evidence_package_id, request_key: b.idempotency_key }); return r.error || reply(r.data, 201); }
